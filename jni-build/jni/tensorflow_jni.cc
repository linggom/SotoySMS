...
TENSORFLOW_METHOD(initText)(JNIEnv* env, jobject thiz, jobject java_asset_manager, jstring model) {
...
static int processtext(const int* raws) {
	// Create input tensor
	Tensor input_tensor( tensorflow::DT_FLOAT,
						 tensorflow::TensorShape( {1, TF_SIZE} ) );

	auto input_tensor_mapped = input_tensor.tensor<float, 2>();

	for(int i= 0; i<TF_SIZE; ++i) {
		int raw = raws[i];
		input_tensor_mapped(0, i) = raw;
	}
...
JNIEXPORT jint JNICALL
TENSORFLOW_METHOD(detectText)(JNIEnv* env, jobject thiz, jintArray rawsText) {
	jboolean iCopied = JNI_FALSE;
	jint* raws = env->GetIntArrayElements(rawsText, &iCopied);

	int result = processtext( reinterpret_cast<int*>(raws) );

	env->ReleaseIntArrayElements(rawsText, raws, JNI_ABORT);
